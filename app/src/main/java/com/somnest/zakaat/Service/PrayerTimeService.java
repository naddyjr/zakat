package com.somnest.zakaat.Service;
...
import com.somnest.zakaat.Appcompany.App;
import com.somnest.zakaat.DatabaseHelper.DatabaseAccess;
import com.somnest.zakaat.MainActivity;
import com.somnest.zakaat.Model.DuaModel;
import com.somnest.zakaat.PrayerAlarmActivity;
import com.somnest.zakaat.PrayerTimeCal.PrayerTime;
...
import com.somnest.zakaat.TodayDuaActivity;
...
public class PrayerTimeService extends Service {
    ...
    PendingIntent activity = PendingIntent.getActivity(
        ...
        Build.VERSION.SDK_INT >= 31 ? PendingIntent.FLAG_MUTABLE : PendingIntent.FLAG_ONE_SHOT
    );

    NotificationCompat.Builder notificationBuilder = new NotificationCompat.Builder(this, App.CHANNEL_ID)
        .setContentTitle("")
        .setPriority(NotificationCompat.PRIORITY_LOW)
        .setContentText("")
        .setSmallIcon(R.drawable.noti_icon)
        .setContentIntent(activity)
        .setCategory(Context.ALARM_SERVICE)
        .setVisibility(NotificationCompat.VISIBILITY_SECRET);

    if (Build.VERSION.SDK_INT >= 26) {
        startForeground(2, notificationBuilder.build());
    } else {
        ((NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE)).notify(2, notificationBuilder.build());
    }
}
